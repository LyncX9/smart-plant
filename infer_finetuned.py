...
def load_finetuned_model(model_path):
    ...
    # Must match classifier in train_finetune.py !
...
# Must match validation transform in train_finetune.py !
transform = T.Compose([
    T.ToPILImage(),
    T.Resize((224,224)),
    T.ToTensor(),
...
def analyze_plant_finetuned(model, image_folder, out_dir="outputs_finetuned"):
...
if __name__ == "__main__":
    if os.path.exists("best_model_finetuned.pth"):
        ...
        model = load_finetuned_model("best_model_finetuned.pth")
        result = analyze_plant_finetuned(model, "sample_leaves")
        print(json.dumps(result, indent=2))
    else:
        print("Model file 'best_model_finetuned.pth' not found. Please run training first!")
